      ...
      try {
        ...
        if (res.ok) {
          try {
            ...
            await db.prepare(
              "INSERT INTO email_log (id, order_id, sent_to, subject, resend_email_id, body_html, kind) VALUES (?, ?, ?, ?, ?, ?, 'refund')"
            ).bind(crypto.randomUUID(), order.id, customerTo, `Refund processed: ${docNumber}`, resendEmailId, html).run();
          } catch (e) {
